...
describe('terminal lifecycle', () => {
    ...
    it.each([false, true])('opens folders in the side panel regardless of the server flag: %s', (enabled) => {
    ...
    it('boots and keeps the active session when the server disables the flag', async () => {
        ...
        expect(window.posthogTerminal).not.toBeUndefined()
    ...
    it('opens the selected folder on first boot and changes it in an existing terminal', async () => {
        ...
        expect(terminalDockLogic.values.dockOpen).toBe(true)
        await waitFor(() => expect(runtime.changeDirectory).toHaveBeenCalledWith('/posthog/files/Hidden'))
    })
