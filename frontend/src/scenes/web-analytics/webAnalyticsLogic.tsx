...
export const webAnalyticsLogic: LogicWrapper<webAnalyticsLogicType> = kea<webAnalyticsLogicType>([
    props({} as WebAnalyticsLogicProps),
    key((props) => props.context ?? 'web-analytics'),
    path((key) => ['scenes', key === 'page-visibility' ? 'pageVisibility' : 'webAnalytics', 'webAnalyticsSceneLogic']),
    ...
    reducers(({ props }) => {
        const persistConfig = buildTeamScopedPersistenceConfig(
            props.context === 'page-visibility' ? 'page_visibility__' : ''
        )
    ...
    tabAwareActionToUrl(({ values, cache, props }) => {
        ...
        const stateToUrl = (): string | undefined => {
            ...
            const isMarketingRoute = router.values.location.pathname.endsWith('/marketing')
            if (
                (props.context === 'page-visibility'
                    ? !isMarketingRoute || router.values.searchParams.tab !== 'page-visibility'
                    : isMarketingRoute) ||
                cache.applyUrlStateDepth > 0
            ) {
                return undefined
            }
    ...
    tabAwareUrlToAction(({ actions, values, cache, props }): UrlToActionPayload<webAnalyticsLogicType> => {
        const applyUrlState = (
        ...
        if (props.context === 'page-visibility') {
            return {
