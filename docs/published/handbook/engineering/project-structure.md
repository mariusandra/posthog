...
The desktop build always enables `simple-sidepanel` and `posthog-terminal` for everyone.
It uses the shared Apps, Files, and Chat sidebar with Code as its fourth tab.
Terminal is available from the app menu and dock.
Opening a sidebar tab while collapsed temporarily expands it over the page; Chat expands the sidebar in place.
