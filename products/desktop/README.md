...
Upstream now has a different, standalone app at `products/desktop`. This fork keeps its Electron shell in the root pnpm workspace, uses the root Oxlint and Oxfmt configuration, and excludes upstream's desktop-only registry outputs from `bin/build-object-tags-registry.py`. Inherited CI definitions can be refreshed during a manual sync; the daily job preserves the checked-in workflow tree.
