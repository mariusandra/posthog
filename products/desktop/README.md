...
This fork's release credentials and installers are separate from upstream's desktop app. The two workflows below are gated to the fork (`if: github.repository == 'mariusandra/posthog'`) and are inert upstream.

Upstream now has a different, standalone app at `products/desktop`. This fork keeps its Electron shell in the root pnpm workspace, uses the root Oxlint and Oxfmt configuration, and excludes upstream's desktop-only registry outputs from `bin/build-object-tags-registry.py`. Inherited CI definitions can be refreshed during a manual sync; the weekly job preserves the checked-in workflow tree.

- `desktop-sync.yml` runs every Monday at 05:23 UTC. It merges two upstream sources into `desktop`: PostHog master (keeps the app current) and the open desktop PR branch `posthog-code/desktop-electron-app` (ongoing desktop dev, skipped once that PR merges). The OpenAI Codex CLI resolves conflicts and merge fallout following the `syncing-desktop-fork` skill (`.agents/skills/syncing-desktop-fork/SKILL.md`). Sync is one-directional (PR branch and master flow into the fork, not back). Desktop tests, desktop typecheck, and frontend typecheck gate every changed sync. The sync preserves the fork’s workflow definitions and Electron app, including fork-deleted files changed upstream. It verifies that both requested refs are included before pushing. Backend and e2e suites do not run on this fork.
- `desktop-release.yml` runs after each sync (and on pushes to `desktop`). It selects the next `year.month.index` version using the UTC release date and existing GitHub releases, starting the index at zero each month. For example: `2026.9.0`, `2026.9.1`, then `2026.10.0` when October starts. It updates `products/desktop/package.json` before building the signed and notarized macOS DMG and the Windows installer, then publishes them under the `desktop-v<version>` tag. A failed build that publishes no release does not consume an index.
