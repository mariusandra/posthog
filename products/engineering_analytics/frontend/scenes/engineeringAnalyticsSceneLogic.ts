import { MakeLogicType, connect, kea, path, selectors } from 'kea'

import { tabAwareScene } from 'lib/logic/scenes/tabAwareScene'

import { sceneLogic } from '~/scenes/sceneLogic'

/** The root scene's views: the repo hub landing, the PR list page, the workflow list page, test health, teams, and engineering health (the DORA metric family). */
export type EngineeringAnalyticsView = 'hub' | 'pull-requests' | 'workflows' | 'test-health' | 'teams' | 'health'

export const VIEW_DESCRIPTIONS: Record<EngineeringAnalyticsView, string> = {
    hub: 'CI health, pull requests, workflows, and cost for the connected repo.',
    'pull-requests': 'Pull requests and their CI status across connected repos.',
    workflows: 'Run volume, success rate, and duration per workflow over the selected window.',
    'test-health': 'Quarantine debt by team: which quarantined tests each team owns, and how long they have stood.',
    teams: 'CI test health by owning team, from the owners.yaml map.',
    health: 'DORA-style deploy metrics from the GitHub deployments data: frequency, merge to deploy lead time, and failure proxies.',
}

const SCENE_KEY_TO_VIEW: Record<string, EngineeringAnalyticsView> = {
    engineeringAnalytics: 'hub',
    engineeringAnalyticsPullRequestList: 'pull-requests',
    engineeringAnalyticsWorkflows: 'workflows',
    engineeringAnalyticsTestHealth: 'test-health',
    engineeringAnalyticsTeams: 'teams',
    engineeringAnalyticsHealth: 'health',
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface engineeringAnalyticsSceneLogicValues {
    sceneKey: string | undefined // sceneLogic
    activeView: EngineeringAnalyticsView
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface engineeringAnalyticsSceneLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        activeView: (sceneKey: string | undefined) => EngineeringAnalyticsView
    }
}

export type engineeringAnalyticsSceneLogicType = MakeLogicType<
    engineeringAnalyticsSceneLogicValues,
    {},
    Record<string, any>,
    engineeringAnalyticsSceneLogicMeta
>

export const engineeringAnalyticsSceneLogic = kea<engineeringAnalyticsSceneLogicType>([
    path(['products', 'engineering_analytics', 'frontend', 'scenes', 'engineeringAnalyticsSceneLogic']),
    tabAwareScene(),
    connect(() => ({
        values: [sceneLogic, ['sceneKey']],
    })),
    selectors({
        activeView: [
            (s) => [s.sceneKey],
            (sceneKey: string): EngineeringAnalyticsView => SCENE_KEY_TO_VIEW[sceneKey] ?? 'hub',
        ],
    }),
])
