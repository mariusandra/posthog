...
// Every metric on this tab counts syncs or synced rows, so there's nothing to show for a direct
// connect source — it queries the origin database instead of syncing into the warehouse.
export function shouldShowManagedSourceMetricsTab(
    source: Pick<ExternalDataSource, 'access_method'> | null | undefined,
    metricsFeatureFlagEnabled: boolean
): boolean {
    return metricsFeatureFlagEnabled && shouldShowManagedSourceSyncsTab(source)
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface sourceSceneLogicValues {
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface sourceSceneLogicActions {
    _setCurrentTab: (tab: SourceSceneTab) => {
        tab: 'configuration' | 'destinations' | 'history' | 'metrics' | 'schemas' | 'syncs' | 'webhook'
    }
    setBreadcrumbName: (name: string) => {
        name: string
    }
    setCurrentTab: (tab: SourceSceneTab) => {
        tab: 'configuration' | 'destinations' | 'history' | 'metrics' | 'schemas' | 'syncs' | 'webhook'
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface sourceSceneLogicMeta {
    key: string
...
export type sourceSceneLogicType = MakeLogicType<
    sourceSceneLogicValues,
    sourceSceneLogicActions,
    SourceSceneProps,
    sourceSceneLogicMeta
>
...
export const sourceSceneLogic = kea<sourceSceneLogicType>([
    ...
    selectors({
        ...
        breadcrumbs: [
            (s) => [s.breadcrumbName],
            (breadcrumbName: string): Breadcrumb[] => {
                return [
                    {
                        key: Scene.Sources,
                        name: 'Sources',
...
function ManagedSourceTabs({
    ...
    const showDestinationsTab = !!featureFlags[FEATURE_FLAGS.WAREHOUSE_MULTI_DESTINATION]
    const showMetricsTab = shouldShowManagedSourceMetricsTab(source, !!featureFlags[FEATURE_FLAGS.DWH_SOURCE_METRICS])
    ...
    if (showDestinationsTab) {
        tabs.push({ label: 'Destinations', key: 'destinations', content: <DestinationsTab id={sourceId} /> })
    }
