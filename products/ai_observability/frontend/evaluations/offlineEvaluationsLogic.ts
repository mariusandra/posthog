...
function parseOfflineDateFilter(dateFilter: OfflineDateFilter): { dateFrom: string | null; dateTo: string | null } {
    return {
        dateFrom: dateFilter.dateFrom ? (dateStringToDayJs(dateFilter.dateFrom)?.toISOString() ?? null) : null,
        dateTo: dateFilter.dateTo ? (dateStringToDayJs(dateFilter.dateTo)?.toISOString() ?? null) : null,
    }
}

function getOfflineDateClauses(dateFilter: OfflineDateFilter): { dateFromClause: string; dateToClause: string } {
    const { dateFrom, dateTo } = parseOfflineDateFilter(dateFilter)

    return {
        dateFromClause: dateFrom ? `AND timestamp >= parseDateTimeBestEffort('${dateFrom}')` : '',
        dateToClause: dateTo ? `AND timestamp <= parseDateTimeBestEffort('${dateTo}')` : '',
    }
}
...
function isOnOfflineEvaluationsRoute(pathname: string): boolean {
    ...
    const offlinePath = urls.aiObservabilityOfflineEvaluations()
...
export function mapOfflineExperimentItems(rows: RawOfflineExperimentMetricRow[]): OfflineExperimentData {
    ...
    for (const row of rows) {
        ...
        const targetId = asString(row[18])
        const targetType = asString(row[19])
...
export interface offlineEvaluationsLogicValues {
    filteredOfflineExperimentItems: OfflineExperimentItem[]
    filteredOfflineExperiments: OfflineExperiment[]
    offlineDateFilter: {
        dateFrom: string | null
        dateTo: string | null
    }
    offlineExperimentItemsFilter: string
    offlineExperiments: OfflineExperiment[]
    offlineExperimentsFilter: string
    offlineExperimentsLoading: boolean
    offlineMetricColumns: OfflineMetricColumn[]
    selectedExperiment: OfflineExperiment | null
    selectedExperimentData: OfflineExperimentData
    selectedExperimentDataLoading: boolean
    selectedExperimentId: string | null
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface offlineEvaluationsLogicActions {
    clearSelectedExperiment: () => {
        value: true
    }
    loadOfflineExperiments: () => any
    loadOfflineExperimentsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    loadSelectedExperimentData: (payload?: { experimentId?: string }) => {
        experimentId?: string
    }
    loadSelectedExperimentDataFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    loadSelectedExperimentDataSuccess: (
        selectedExperimentData: OfflineExperimentData,
        payload?: {
            experimentId?: string
        }
    ...
    setOfflineExperimentItemsFilter: (filter: string) => {
        filter: string
    }
    setOfflineExperimentsFilter: (filter: string) => {
        filter: string
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface offlineEvaluationsLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        selectedExperiment: (
            offlineExperiments: OfflineExperiment[],
            selectedExperimentId: string | null
        ) => OfflineExperiment | null
        filteredOfflineExperiments: (
            offlineExperiments: OfflineExperiment[],
            offlineExperimentsFilter: string
        ) => OfflineExperiment[]
        offlineMetricColumns: (selectedExperimentData: OfflineExperimentData) => OfflineMetricColumn[]
...
export type offlineEvaluationsLogicType = MakeLogicType<
    offlineEvaluationsLogicValues,
    offlineEvaluationsLogicActions,
    OfflineEvaluationsLogicProps,
    offlineEvaluationsLogicMeta
>
...
export const offlineEvaluationsLogic = kea<offlineEvaluationsLogicType>([
                ...
                loadOfflineExperiments: async () => {
                    ...
                    try {
                        const response = await api.queryHogQL(
                            ...
                            `,
                            { productKey: 'llm_analytics', scene: 'AIObservabilityEvaluations' }
                        )
        ...
        selectedExperimentData: [
            getEmptyOfflineExperimentData(),
            {
                loadSelectedExperimentData: async (payload: { experimentId?: string } = {}) => {
                    ...
                    try {
                        const { dateFrom, dateTo } = parseOfflineDateFilter(values.offlineDateFilter)
                        const response = await aiObservabilityOfflineEvaluationsExperimentItemsCreate(
    ...
    selectors({
        ...
        offlineMetricColumns: [
            ...
            (selectedExperimentData: OfflineExperimentData) => selectedExperimentData.metricColumns,
        ],
    ...
    tabAwareUrlToAction(({ actions, values }) => ({
        ...
        [urls.aiObservabilityOfflineEvaluationExperiment(':experimentId', false)]: ({ experimentId }, searchParams) => {
    ...
    tabAwareActionToUrl(() => ({
        ...
        selectExperiment: ({ experimentId }) => [
            urls.aiObservabilityOfflineEvaluationExperiment(experimentId),
            {
                ...router.values.searchParams,
                tab: undefined,
