...
export function openInPlayground(payload: PlaygroundSetupPayload): void {
    ...
    router.actions.push(urls.aiObservabilityPlayground())
}
...
export interface llmPlaygroundPromptsLogicValues {
    ...
    editModal: {
        messageIndex?: number
        promptId: string
        type: 'message' | 'system' | 'tools'
    } | null
    hasRunnablePrompts: boolean
    linkedSource: {
        evaluationId: string | null
        evaluationName: string | null
        promptName: string | null
        promptVersion: number | null
        type: 'evaluation' | 'prompt' | null
    }
    localToolsJsonByPromptId: Record<string, string | null>
    maxTokens: number | null
    messages: Message[]
    model: string
    ...
    saving: boolean
    selectedProviderKeyId: string | null
    sourceSetupLoading: boolean
    systemPrompt: string
    temperature: number | null
    thinking: boolean
    tools: Record<string, unknown>[] | null
    toolsJsonErrorByPromptId: Record<string, string | null>
    topP: number | null
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface llmPlaygroundPromptsLogicActions {
    addMessage: (
    ...
    addPromptConfig: (sourcePromptId?: string) => {
        newPromptId: string
        sourcePromptId: string | undefined
    }
    ...
    setSourceSetupLoading: (isLoading: boolean) => {
        isLoading: boolean
    }
    setSystemPrompt: (
        systemPrompt: string,
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface llmPlaygroundPromptsLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        activePromptConfig: (promptConfigs: PromptConfig[], activePromptId: string | null) => PromptConfig
        model: (activePromptConfig: PromptConfig) => string
        selectedProviderKeyId: (activePromptConfig: PromptConfig) => string | null
        systemPrompt: (activePromptConfig: PromptConfig) => string
        maxTokens: (activePromptConfig: PromptConfig) => number | null
        thinking: (activePromptConfig: PromptConfig) => boolean
        temperature: (activePromptConfig: PromptConfig) => number | null
        topP: (activePromptConfig: PromptConfig) => number | null
        reasoningLevel: (activePromptConfig: PromptConfig) => ReasoningLevel
        tools: (activePromptConfig: PromptConfig) => Record<string, unknown>[] | null
        messages: (activePromptConfig: PromptConfig) => Message[]
        linkedSource: (promptConfigs: PromptConfig[]) => {
            evaluationId: string | null
            evaluationName: string | null
            promptName: string | null
            promptVersion: number | null
        ...
        hasRunnablePrompts: (promptConfigs: PromptConfig[]) => boolean
    }
}

export type llmPlaygroundPromptsLogicType = MakeLogicType<
    llmPlaygroundPromptsLogicValues,
    llmPlaygroundPromptsLogicActions,
    LLMPlaygroundPromptsLogicProps,
    llmPlaygroundPromptsLogicMeta
>
    ...
    listeners(({ actions, values, props }) => ({
        ...
        setupPlaygroundFromEvent: async ({ payload }) => {
            ...
            const finishSourceSetup = (sourceParam: Record<string, string>): void => {
                ...
                router.actions.replace(
                    combineUrl(urls.aiObservabilityPlayground(), { ...cleanParams, ...sourceParam }).url
                )
            }
        ...
        saveToLinkedPrompt: async ({ promptId }) => {
            ...
            try {
                ...
                if (linkedSource.promptVersion) {
                    ...
                    router.actions.replace(combineUrl(urls.aiObservabilityPlayground(), cleanParams).url)
                }
                ...
                lemonToast.success(`${label.charAt(0).toUpperCase()}${label.slice(1)} updated`, {
                    button: {
                        label: 'View',
                        action: () => router.actions.push(urls.aiObservabilityPrompt(promptName)),
                    },
                })
        ...
        saveToLinkedEvaluation: async ({ promptId, modelConfig }) => {
            ...
            try {
                ...
                lemonToast.success(`${label.charAt(0).toUpperCase()}${label.slice(1)} updated`, {
                    button: {
                        label: 'View',
                        action: () => router.actions.push(urls.aiObservabilityEvaluation(evalId)),
                    },
                })
        ...
        saveAsNewPrompt: async ({ promptId, name }) => {
            ...
            try {
                ...
                lemonToast.success('Prompt saved', {
                    button: {
                        label: 'View',
                        action: () => router.actions.push(urls.aiObservabilityPrompt(name)),
                    },
                })
            } catch (error: unknown) {
        ...
        saveAsNewEvaluation: async ({ promptId, name, modelConfig }) => {
            ...
            try {
                ...
                lemonToast.success('Evaluation saved', {
                    button: {
                        label: 'View',
                        action: () => router.actions.push(urls.aiObservabilityEvaluation(created.id)),
                    },
                })
            } catch (error: unknown) {
