...
export interface aiObservabilityScoreDefinitionsLogicValues {
    currentProjectId: number | null // projectLogic
    archivingDefinitionIds: Set<string>
    count: number
    filters: ScoreDefinitionFilters
    isArchivingDefinition: (definitionId: string) => boolean
    modalMode: ScoreDefinitionModalMode | null
    pagination: PaginationManual
    rawFilters: Partial<ScoreDefinitionFilters> | null
    scoreDefinitionCountLabel: string
    scoreDefinitions: PaginatedScoreDefinitionListApi
    scoreDefinitionsLoading: boolean
    selectedDefinition: ScoreDefinitionApi | null
    sorting: Sorting | null
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface aiObservabilityScoreDefinitionsLogicActions {
    closeModal: () => {
        value: true
    }
    loadScoreDefinitions: (debounce?: boolean) => {
        debounce: boolean
    }
    loadScoreDefinitionsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    setFilters: (
        filters: Partial<ScoreDefinitionFilters>,
        merge?: boolean,
        debounce?: boolean
    ) => {
        debounce: boolean
    ...
    toggleArchive: (scoreDefinition: ScoreDefinitionApi) => {
        scoreDefinition: ScoreDefinitionApi
    }
    toggleArchiveFailure: (definitionId: string) => {
        definitionId: string
    }
    toggleArchiveSuccess: (definitionId: string) => {
        definitionId: string
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface aiObservabilityScoreDefinitionsLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        filters: (rawFilters: Partial<ScoreDefinitionFilters> | null) => ScoreDefinitionFilters
        count: (scoreDefinitions: PaginatedScoreDefinitionListApi) => number
        sorting: (filters: ScoreDefinitionFilters) => Sorting | null
        pagination: (filters: ScoreDefinitionFilters, count: number) => PaginationManual
        scoreDefinitionCountLabel: (
            filters: ScoreDefinitionFilters,
            count: number,
            scoreDefinitionsLoading: boolean
        ) => string
        isArchivingDefinition: (archivingDefinitionIds: Set<string>) => (definitionId: string) => boolean
    }
}

export type aiObservabilityScoreDefinitionsLogicType = MakeLogicType<
    aiObservabilityScoreDefinitionsLogicValues,
    aiObservabilityScoreDefinitionsLogicActions,
    AIObservabilityScoreDefinitionsLogicProps,
    aiObservabilityScoreDefinitionsLogicMeta
>
...
export const aiObservabilityScoreDefinitionsLogic = kea<aiObservabilityScoreDefinitionsLogicType>([
            ...
            { results: [], count: 0, next: null, previous: null } as PaginatedScoreDefinitionListApi,
            {
                loadScoreDefinitions: async ({ debounce }, breakpoint) => {
                    ...
                    if (!values.currentProjectId) {
                        return values.scoreDefinitions
                    }
    ...
    selectors({
        ...
        isArchivingDefinition: [
            ...
            (archivingDefinitionIds: Set<string>): ((definitionId: string) => boolean) => {
