...
export const aiObservabilityDashboardLogic = kea<aiObservabilityDashboardLogicType>([
    ...
    connect((props: AIObservabilityDashboardLogicProps) => ({
        ...
        actions: [aiObservabilitySharedLogic({ tabId: props.tabId }), ['setSavedDashboardDates']],
    })),
                    ...
                    return await Promise.all(
                        dashboards.map(async (dashboardSummary) => {
                            ...
                            return {
                                ...
                                dateFrom:
                                    dashboard.persisted_filters && 'date_from' in dashboard.persisted_filters
                                        ? (dashboard.persisted_filters.date_from ?? null)
                                        : '-7d',
                                dateTo: dashboard.persisted_filters?.date_to ?? null,
                            }
                        })
                    )
                },
        ...
        loadLLMDashboardsSuccess: async ({ availableDashboards }, breakpoint) => {
            if (availableDashboards.length === 0) {
                ...
                const selectedDashboard =
                    availableDashboards.find(({ id }) => id === values.selectedDashboardId) ?? availableDashboards[0]
                actions.setSavedDashboardDates(selectedDashboard.dateFrom, selectedDashboard.dateTo)
            }
        },
    ...
    selectors({
        ...
        // products/ai_observability/backend/dashboard_templates.py:4-319 until full migration to customizable dashboard.
        //
                    ...
                    context: {
                        ...
                        onDataPointClick: (series) => {
                            if (typeof series.day === 'string') {
                                ...
                                router.actions.push(urls.aiObservabilityUsers(), {
                                    ...router.values.searchParams,
                                    date_from,
                                    date_to,
                                })
