...
describe('aiObservabilityTraceLogic', () => {
    let logic: ReturnType<typeof aiObservabilityTraceLogic.build>
    ...
    it('properly loads trace scene when trace ID contains a colon', async () => {
        ...
        const traceUrl = combineUrl(urls.aiObservabilityTrace(traceIdWithColon))
    ...
    it('handles trace ID with event and timestamp parameters', async () => {
        ...
        const traceUrl = combineUrl(urls.aiObservabilityTrace(traceIdWithColon, { event: eventId, timestamp }))
    ...
    it('handles trace ID with event and exception_ts parameters', async () => {
        ...
        const traceUrl = combineUrl(urls.aiObservabilityTrace(traceIdWithColon, { event: eventId, exception_ts }))
    ...
    it('preserves timestamp when both timestamp and tab parameters are present', async () => {
        ...
        const traceUrl = combineUrl(urls.aiObservabilityTrace(traceId, { timestamp, tab: 'conversation' }))
    ...
    describe('setSearchQuery URL updates', () => {
        ...
        it('removes search param from URL when query is cleared', async () => {
            ...
            expect(routerSpy).toHaveBeenCalledWith(urls.aiObservabilityTrace('test-trace-123', {}))
        })
