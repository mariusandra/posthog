...
export interface clustersLogicValues {
    ...
    availableVerdictCounts: Record<EvaluationVerdict, number>
    breadcrumbs: Breadcrumb[]
    clusterMembershipPredicate: (id: string) => boolean
    ...
    currentRunMatchesLevel: boolean
    effectiveRunId: string | null
    evalFilterEvaluatorNames: string[]
    evalFilterPredicate: (evalId: string) => boolean
    evalFilterVerdicts: EvaluationVerdict[]
    evalFiltersActive: boolean
    evaluationItemAttributes: Record<string, EvaluationItemAttributes>
    expandedClusterIds: Set<number>
    filteredItemCount: number
    filteredSortedClusters: Cluster[]
    isClusterExpanded: (clusterId: number) => boolean
    isScatterPlotExpanded: boolean
    propertyFilteredItemIds: Set<string> | null
    propertyFilteredItemIdsLoading: boolean
    propertyFiltersActive: boolean
    scatterPlotSeries: ClusterScatterSeries[]
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface clustersLogicActions {
    ...
    loadClusteringRun: (runId: string) => string
    ...
    loadEvaluationAttributesForRun: (run: ClusteringRun) => {
        run: ClusteringRun
    }
    loadPropertyFilteredItemIds: () => {
        value: true
    }
    loadPropertyFilteredItemIdsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    loadPropertyFilteredItemIdsSuccess: (
        propertyFilteredItemIds: Set<string> | null,
        payload?: {
            value: true
        }
    ...
    setEvalEvaluatorNamesFilter: (names: string[]) => {
        names: string[]
    }
    setEvalVerdictsFilter: (verdicts: EvaluationVerdict[]) => {
        verdicts: EvaluationVerdict[]
    }
    setEvaluationItemAttributes: (attributes: Record<string, EvaluationItemAttributes>) => {
        attributes: Record<string, EvaluationItemAttributes>
    }
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface clustersLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        effectiveRunId: (selectedRunId: string | null, clusteringRuns: ClusteringRunOption[]) => string | null
        currentRunMatchesLevel: (currentRun: ClusteringRun | null, clusteringLevel: ClusteringLevel) => boolean
        sortedClusters: (currentRun: ClusteringRun | null, currentRunMatchesLevel: boolean) => Cluster[]
        evalFiltersActive: (
            clusteringLevel: ClusteringLevel,
            evalFilterEvaluatorNames: string[],
            evalFilterVerdicts: EvaluationVerdict[]
        ) => boolean
        availableEvaluatorNames: (evaluationItemAttributes: Record<string, EvaluationItemAttributes>) => {
            count: number
            name: string
        }[]
        availableVerdictCounts: (
            evaluationItemAttributes: Record<string, EvaluationItemAttributes>
        ) => Record<EvaluationVerdict, number>
        evalFilterPredicate: (
            clusteringLevel: ClusteringLevel,
            evaluationItemAttributes: Record<string, EvaluationItemAttributes>,
            evalFilterEvaluatorNames: string[],
        ...
        propertyFiltersActive: (propertyFilters: AnyPropertyFilter[], shouldFilterTestAccounts: boolean) => boolean
        anyFiltersActive: (evalFiltersActive: boolean, propertyFiltersActive: boolean) => boolean
        ...
        filteredItemCount: (filteredSortedClusters: Cluster[]) => number
        totalItemCount: (sortedClusters: Cluster[]) => number
        isClusterExpanded: (expandedClusterIds: Set<number>) => (clusterId: number) => boolean
        scatterPlotSeries: (filteredSortedClusters: Cluster[]) => ClusterScatterSeries[]
    }
}

export type clustersLogicType = MakeLogicType<
    clustersLogicValues,
    clustersLogicActions,
    ClustersLogicProps,
    clustersLogicMeta
>
...
export const clustersLogic = kea<clustersLogicType>([
    ...
    connect((props: ClustersLogicProps) => ({
        values: [
            ...
            llmEvaluationsLogic({ tabId: props.tabId }),
    ...
    selectors({
            ...
            (): Breadcrumb[] => [
                {
                    ...
                    name: 'Clusters',
                    path: urls.aiObservabilityClusters(),
        ...
        scatterPlotSeries: [
            (s) => [s.filteredSortedClusters],
            (clusters: Cluster[]): ClusterScatterSeries[] => {
                const itemSeries: ClusterScatterSeries[] = []
                const centroidSeries: ClusterScatterSeries[] = []
                ...
                for (const cluster of clusters) {
                    ...
                    itemSeries.push({
                        ...
                        color,
                        pointRadius: 4,
                        shape: isOutlier ? 'cross' : 'circle', // X shape for outliers
                        points: Object.entries(cluster.traces).map(([itemKey, itemInfo]) => ({
                            x: itemInfo.x,
                            y: itemInfo.y,
                            meta: {
                            ...
                            points: [
                                {
                                    ...
                                    meta: { clusterId: cluster.cluster_id, isCentroid: true },
                                },
                            ],
                        })
                    }
                ...
                return [...itemSeries, ...centroidSeries]
            },
        ],
    }),
    ...
    tabAwareActionToUrl(({ values }) => ({
        setSelectedRunId: () => {
            ...
            const pathname = values.selectedRunId
                ? urls.aiObservabilityClusters(values.selectedRunId)
                : urls.aiObservabilityClusters()
